...
class TOODHead(ATSSHead):
    ...
    def __init__(self,
        ...
        if self.train_cfg:
            ...
            ############################我添加的内容
            #self.reduce_channels = nn.Conv2d(1536, 256, kernel_size=1)
    ...
    def forward(self, feats):
        ...
        for idx, (x, scale, stride) in enumerate(
            ...
            # feat = self.reduce_channels(feat)
            avg_feat = F.adaptive_avg_pool2d(feat, (1, 1))
...
#######################################################
            #cls prediction and alignment  分类预测和对齐
            ...
            if self.anchor_type == 'anchor_free':
                reg_dist = scale(self.tood_reg(reg_feat).exp()).float() # reg_feat 改为feat
                ...
                reg_dist = scale(self.tood_reg(reg_feat)).float() # 同上
                reg_dist = reg_dist.permute(0, 2, 3, 1).reshape(-1, 4)
